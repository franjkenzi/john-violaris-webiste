  ...
  return (
    <form ref={formRef} action={formAction} className="space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {fee ? <DeleteFee id={fee.id} title={fee.values.title} /> : null}
      </div>
    </form>
  );
}
...
/**
 * Deletion is permanent, so it asks first. The confirm sits on the form's
 * submit event rather than the button's click, so a keyboard submit is caught
 * too. A sibling form rather than a nested one, which is invalid HTML.
 */
function DeleteFee({ id, title }: { id: string; title: string }) {
  return (
    <form
      action={deleteFee}
      onSubmit={(event) => {
        const confirmed = window.confirm(
          `Permanently delete the "${title}" fee? This cannot be undone — to take it off the site and keep it, unpublish it instead.`,
        );
